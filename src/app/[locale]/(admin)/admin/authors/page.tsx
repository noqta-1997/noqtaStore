...
export async function generateMetadata({
  params,
}: AdminAuthorsPageProps): Promise<Metadata> {
...
export default async function AdminAuthorsPage({
  ...
  const authors = term
    ? allAuthors.filter((author) =>
        `${author.name.ar} ${author.name.en} ${author.slug}`
          .toLowerCase()
            ...
            <Tbody>
              {authors.map((author) => (
                <Tr key={author.id}>
                  ...
                  <Td>
                    <RowActions
                      viewHref={`/${locale}/authors/${author.slug}`}
                      editHref={`/${locale}/admin/authors/${author.id}/edit`}
