...
import { setHandoutCartQuantity } from "@/app/actions/cart";
import { notifyCartChanged } from "@/lib/cart-signal";
import { cn } from "@/lib/utils";

interface HandoutQuantityStepperProps {
  defaultValue?: number;
  min?: number;
  max?: number;
  labels: { quantity: string; increase: string; decrease: string };
  /** The handout cart line the changes are written to. */
  handoutId: string;
  className?: string;
}
...
export function HandoutQuantityStepper({
  ...
  const change = (next: number) => {
    ...
    timer.current = setTimeout(async () => {
      ...
      await setHandoutCartQuantity(handoutId, next);
      notifyCartChanged();
      router.refresh();
    }, WRITE_DELAY);
  };
