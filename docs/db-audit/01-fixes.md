...
>
> **الفترة:** 2026-09-16 → 2026-09-18. **الالتزامات:** من `6ca656c` إلى `cfeceda` (27 التزاماً). **الترحيلات المضافة:** 10 (من `20260916160000_unique_catalogue_names` إلى `20260918110000_lock_public_api`)؛ سجل القاعدة والمجلد متطابقان (23 ترحيلاً).
>
...
| 22 | `7256ab6` | كل `public` | الجداول كلها مكشوفة للقراءة والكتابة عبر REST بالمفتاح العلني | RLS على الـ23 جدولاً بلا سياسات، سحب صلاحيات `anon`/`authenticated` والصلاحيات الافتراضية |
| 23 | `521d351` | إدارة | صفٌ محذوف من تبويب آخر يبقى على الشاشة | `router.refresh()` عند `notFound` في الحوار والنموذج وزر الحظر |
