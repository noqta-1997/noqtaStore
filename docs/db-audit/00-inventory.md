...
>
> **تاريخ الالتقاط:** 2026-09-16 22:04:51 UTC (17/09/2026, 01:04:51 بتوقيت بغداد)  
...
> **حالة المستودع وقت الالتقاط:** الالتزام `574bc24` (في الشجرة 6 ملف غير ملتزم).  
...
| إضافات (extensions) | 5 |
| إجمالي الصفوف في كل الجداول | 814 |
...
|--:|---|---|--:|--:|--:|--:|---|
| 1 | `_prisma_migrations` | `MIGRATIONS` | 8 | 18 | 18 | 32 kB | سجل ترحيلات Prisma |
...
| 6 | `mfa_amr_claims` | `AUTH_CORE` | 5 | 129 | 129 | 88 kB |  |
...
| 16 | `refresh_tokens` | `AUTH_CORE` | 9 | 142 | 142 | 168 kB |  |
...
| 22 | `sessions` | `AUTH_CORE` | 15 | 129 | 129 | 128 kB |  |
...
| `AUTH_CORE` | الهوية والجلسات | `auth` | `users`، `identities`، `sessions`، `refresh_tokens`، `mfa_amr_claims`، `one_time_tokens`، `flow_state`، `audit_log_entries`، `instances` | 9 | 410 |
...
| `MIGRATIONS` | سجلات الترحيل | متعدد | `auth.schema_migrations`، `public._prisma_migrations`، `realtime.schema_migrations`، `storage.migrations` | 4 | 250 |
...
أربعة دفاتر مستقلة: `public._prisma_migrations` (18 صفاً — يخصّ التطبيق، الملحق هـ)، و`auth.schema_migrations` (82)، `realtime.schema_migrations` (82)، `storage.migrations` (68) تخصّ خدمات Supabase.
...
|---|---|---|
| عدد الترحيلات | 18 | 18 |
| آخر ترحيل | `20260917120000_coupon_usage_check` | `20260917120000_coupon_usage_check` (اكتمل 2026-09-16 22:02:16 UTC) |
...
### `_prisma_migrations` — 8 عموداً، 18 صف، مجموعة `MIGRATIONS`
